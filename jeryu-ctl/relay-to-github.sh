#!/usr/bin/env bash
# jeryu-ctl/relay-to-github.sh — manual legacy relay: push a jeryu-main commit
...
# unless a human passes --manual or sets ALLOW_LEGACY_RELAY=1.
#
...
[[ "$MANUAL" == "1" || "${ALLOW_LEGACY_RELAY:-0}" == "1" ]] \
  || die "legacy GitHub relay is manual-only; pass --manual or set ALLOW_LEGACY_RELAY=1"
bare="$(bare_path "$OWNER" "$REPO")"; [[ -d "$bare" ]] || die "no bare repo: $bare"
guard_no_insteadof                       # never let a 2224/gitea rewrite hijack this push
tok="$(github_token)"; [[ -n "$tok" ]] || die "no github token (GH_RELAY_TOKEN / .git-credentials-jeryu / gh auth)"
...
git -C "$bare" push "$url" "$SHA:refs/heads/$branch" 2>&1 | grep -vi 'x-access-token' | tail -3 || die "relay push failed"
gh pr create --repo "$SLUG" --base main --head "$branch" $DRAFT \
  --title "jeryu relay: $short" \
  --body "Manual legacy relay of jeryu-merged commit \`$SHA\`. Jeryu-local checks remain the source of truth." \
